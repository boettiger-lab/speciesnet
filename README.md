...
The `speciesnet` package provides an R wrapper for Google’s
[SpeciesNet](https://github.com/google/cameratrapai) camera trap AI
model, allowing users to classify species in camera trap images using
deep learning from within R.

SpeciesNet provides species-level classification (not just
animal/person/vehicle detection) and includes the following key
features:
...
The package requires a Python environment. It will automatically install
the necessary `speciesnet` python package:
...
SpeciesNet is developed by Google Research and provides state-of-the-art
species classification for camera trap images. The model:
...
For more information, see the [official SpeciesNet
repository](https://github.com/google/cameratrapai).
